 ...
 * ! assets, title, price, buttons, & Total price
...
let generateCartItems = () => {
  if (basket.length !== 0) {
    ...
    label.innerHTML = `
    <a href="index.html">
      <button class="HomeBtn">Cart is Empty ⏩⏩⏩ Back to Home</button>
    </a>
    `;
  }
};
...
let TotalAmount = () => {
  if (basket.length !== 0) {
    ...
    <button class="bill"> Total Bill : $ ${amount} </button>
    <button onclick="buynow()" class="buy">Buy Now</button>
...
let buynow = () => {
  alert(
    "Congratulations! 🎉 Your order will be delivered right after this website's production😂😂😂"
  );
};
